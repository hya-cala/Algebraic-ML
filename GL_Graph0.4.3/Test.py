...
T11 = mg.Term(np.array([[1, 1], [1, 1]]), False, 'T4-')
...
for term in Graph.PositiveTerm:
    Graph.SparseCrossing(v,term)
    for son in term.son:
        Graph.SparseCrossing(son,term)
        for atom in Graph.AtomList:
            if atom.son!=[]:
                for son in atom.son:
                    for father in atom.father:
                        Graph.addEdge(father,son)
                        Graph.dualgraph.addEdge(son.dual,father.dual)
                        Graph.removeNode(atom)
'''
for constant in Graph.ConstantList:
    Graph.SparseCrossing(v,constant)
'''
