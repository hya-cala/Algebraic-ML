...
class Term(MatrixNode): 
    def __init__(self, data, attribute, ID=None): 
        ...
        if isinstance(data, np.ndarray):
            ...
            self.data = None
            print("Warning: Please input a ndarray for data of", self.ID)
...
class Graph:
    ...
    def addNode(self, node):
        ...
        #print('add node',node.ID,'finished')
        return self
    ...
    def getAllSon(self, node): 
        #print("in GetAllSon")
        AllSon = node.son.copy()
        if AllSon == []:
            return []

        for son in AllSon:
            AllSon = Add(AllSon, self.getAllSon(son))
        
        return AllSon
    
    #定义GL函数
    def getAllSons(self, node):
        AllSons = self.getAllSon(node).copy()
        AllSons.append(node)
        return AllSons
    
    #定义GU函数
    def getAllFathers(self,node):
        AllFathers = node.father.copy()
        #print([i.ID for i in AllFathers])
        if AllFathers == []:
            return []

        for father in AllFathers:
            AllFathers = Add(AllFathers, self.getAllFathers(father))
        return AllFathers
    
    #定义GLa,GLc函数
    def getSonsByType(self, node, type1):
        getSonsByType = []
        sons = self.getAllSons(node).copy()
        for son in sons:
            if isinstance(son,type1):
